...
// Send is a one-shot client: dial the socket at path, write cmd as a
// single NDJSON line, read one Response, close. Suitable for the dicta
// CLI and for the dicta-preview panel's command channel (where a fresh
// connection per command is fine since panel commands are infrequent).
